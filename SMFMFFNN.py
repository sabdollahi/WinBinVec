...
for cancer_class in classes:
    ...
    while(indices_specific):
        ...
        indices_specific = next(parts_specific, None)
        indices_other = next(parts_other, None)
    if not os.path.exists('RESULTS/SMFMFFNNResults'):
        os.makedirs('RESULTS/SMFMFFNNResults')
    pickle.dump(folds_accuracy, open("RESULTS/SMFMFFNNResults/" + cancer_class + "_Accuracy.pickle","wb"))
    pickle.dump(folds_roc_auc, open("RESULTS/SMFMFFNNResults/" + cancer_class + "_ROC_AUC.pickle","wb"))
    pickle.dump(folds_PR_auc, open("RESULTS/SMFMFFNNResults/" + cancer_class + "_PR_AUC.pickle","wb"))
    ...
    while(indices_metastasis):
        ...
        for shuffled_epoch in range(20):
            ...
            for epoch in range(50):
                for index in range(0, len(training), batch_size):
                    y = Y[index : index + batch_size]
                    batch = tcga_SMFM_dataframe.loc[training[index : index + batch_size]]
                    batch = batch.to_numpy()
                    X = batch[:,0:x_dim].astype(float)
                    X = torch.FloatTensor(X)
                    optimizer.zero_grad()
        ...
        for index in range(0, len(training), batch_size):
            y = Y_test[index : index + batch_size]
            test_list = tcga_SMFM_dataframe.loc[test[index : index + batch_size]]
            test_list = test_list.to_numpy()
            test_list = test_list[:,0:x_dim].astype(float)
            test_list = torch.FloatTensor(test_list)
            if(len(test_list) <= 1):
        ...
        indices_metastasis = next(parts_metastasis, None)
        indices_other = next(parts_other, None)
    if not os.path.exists('RESULTS/SMFMFFNN-StagePrediction'):
        os.makedirs('RESULTS/SMFMFFNN-StagePrediction')
    pickle.dump(folds_accuracy, open("RESULTS/SMFMFFNN-StagePrediction/Part" + str(i) + "_folds_accuracy.pickle","wb"))         
