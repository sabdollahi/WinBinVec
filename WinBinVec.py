...
for cancer_class in classes:
    ...
    while(indices_specific):
        ...
        indices_specific = next(parts_specific, None)
        indices_other = next(parts_other, None)
    if not os.path.exists('RESULTS/WinBinVecResults'):
        os.makedirs('RESULTS/WinBinVecResults')
    pickle.dump(folds_accuracy, open("RESULTS/WinBinVecResults/" + cancer_class + "_Accuracy.pickle","wb"))
    pickle.dump(folds_roc_auc, open("RESULTS/WinBinVecResults/" + cancer_class + "_ROC_AUC.pickle","wb"))
    pickle.dump(folds_PR_auc, open("RESULTS/WinBinVecResults/" + cancer_class + "_PR_AUC.pickle","wb"))
...
for i in range(7):
    ...
    while(indices_metastasis):
        ...
        indices_metastasis = next(parts_metastasis, None)
        indices_other = next(parts_other, None) 
    if not os.path.exists('RESULTS/WinBinVec-StagePrediction'):
        os.makedirs('RESULTS/WinBinVec-StagePrediction')
    pickle.dump(folds_accuracy, open("RESULTS/WinBinVec-StagePrediction/Part" + str(i) + "_folds_accuracy.pickle","wb"))        
